...
namespace FairyGUI
{
    ...
    public class GComponent : GObject
    {
       ...
       public void SetMaterialLayer(int layer)
         {
             List<GImage> imgs = new List<GImage>();
             for (int i = 0; i < _children.Count; i++)
             {
                 GetImageChlid(_children[i], imgs);
             }
             for (int i = 0; i < imgs.Count; i++)
             {
                 Material imaterial = new Material(Shader.Find("FairyGUI/BackGround"));
                 imaterial.renderQueue = layer;
                 imgs[i].material = imaterial;
             }
         }
 
         public void ShowFlew()
         {
             List<GImage> imgs = new List<GImage>();
             for (int i = 0; i < _children.Count; i++)
             {
             ...
             for (int i = 0; i < imgs.Count; i++)
             {
                 Material imaterial = new Material(Shader.Find("FairyGUI/Flow"));
                 imaterial.renderQueue = 3000;
                 imgs[i].material = imaterial;
             }
         }
         ...
         private List<GImage> GetImageChlid(GObject gObject, List<GImage> images)
         {
             PackageItem pi = gObject.packageItem;
             if(pi!=null&&pi.objectType== ObjectType.Image)
             {
                 images.Add(gObject.asImage);
                 ;           }
             if (gObject is GComponent com)
             {
                 if (com.numChildren != 0)
                 {
                     ...
                     {
                         GetImageChlid(com.GetChildAt(j), images);
                     }
                 }
                 else
                 {
